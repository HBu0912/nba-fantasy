...
// BDL/standard abbreviation → ESPN team ID
...
const ESPN_TO_STD: Record<string, string> = {
  GS:"GSW", NY:"NYK", NO:"NOP", SA:"SAS", GOS:"GSW",
};
...
async function fromESPN(
  ...
  // 1. Get ESPN athlete ID from team roster
  ...
  // 2. Fetch team schedule(s) — ESPN uses end-year numbering (2026 = 2025-26 season)
  ...
  // 3. Collect completed events with scores
  ...
  // 4. Fetch game summaries in parallel (cached for completed games)
  ...
  for (const item of summaries) {
    ...
    if (!stats || !names) continue; // DNP or not in game
