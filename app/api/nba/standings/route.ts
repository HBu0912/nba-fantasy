...
// ESPN may use shortened abbreviations — normalize to standard
const ESPN_TO_STD: Record<string, string> = {
...
export async function GET() {
  try {
    ...
    for (const conf of json.children ?? []) {
      const confName: string = (conf.abbreviation ?? conf.name ?? "").toLowerCase().startsWith("e") ? "East" : "West";
      ...
      for (const entry of conf.standings?.entries ?? []) {
        ...
        const getStat = (name: string) =>
          (entry.stats ?? []).find((s: any) => s.name === name || s.shortDisplayName === name);
        const val  = (name: string): number => Number(getStat(name)?.value  ?? 0);
        const disp = (name: string): string => String(getStat(name)?.displayValue ?? "");

        // Records: ESPN encodes home/road/last10 as record entries
        const rec = (id: string) =>
          (entry.records ?? []).find((r: any) => r.id === id)?.summary ?? "-";
